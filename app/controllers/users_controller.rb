class UsersController < ApplicationController
	...
	def user_params
		unless params["user"]["addresses"].blank?
			...
			paramsp["user"].delete("addresses")
		end
		params.fetch(:user, {}).permit(:first_name, :last_name, :email, :phone, :addresses_attributes => [:id, :street1, :street2, :city, :state, :country, :zipcode, :_destroy, :user_id])
	end
